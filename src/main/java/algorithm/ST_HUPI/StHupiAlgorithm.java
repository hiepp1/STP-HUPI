...
public class StHupiAlgorithm {
    ...
    // Constructor.
    public StHupiAlgorithm(List<Transaction> transactions, int k) {
    ...
    private List<Itemset> generateItemsets() {
        ...
        StHupiTree root = new StHupiTree(new ArrayList<>(), 0, 0f, 0f, 0);
...
//                if (expectedUtility < 0) continue;
                ...
                StHupiTree node = new StHupiTree(currentItemset, utility, expectedUtility, 0, 0);
                Map<Integer, StHupiTree> children = node.getChildren();
                children.put(item, node);
                root.setChildren(children);

                this.stHupiTreeGrowth(node);
            }
        }
     ...
     */
    private void stHupiTreeGrowth(StHupiTree node) {
        ...
        for (Integer item : extensionItems) {
            ...
            if (psu >= this.minUtil) {
                ...
                StHupiTree childNode = new StHupiTree(new ArrayList<>(newItemset),
                        newUtility, newTotalExpUtil, newPosUtility, newNegUtility);
//                node.children.put(item, childNode);

                Map<Integer, StHupiTree> children = childNode.getChildren();
                children.put(item, childNode);
                node.setChildren(children);
                this.stHupiTreeGrowth(childNode);
            }
        }
    }
     ...
     */
    private void processCurrentItemset(StHupiTree node) {
        ...
        Itemset itemset = new Itemset(currentItemset, node.getUtility(), node.getExpectedUtility(), 0);
